...
const size_t _numt = _dynamic_array_statemonitor_t.size();
    ...
    _dynamic_array_statemonitor_y.resize(_new_size, _num_indices);
    ...
    for (int _i = 0; _i < (int)_num_indices; _i++)
    {
        ...
        const double _source_y = _ptr_array_neurongroup_y[_idx];
        const double _to_record_y = _source_y;


        _dynamic_array_statemonitor_y(_new_size-1, _i) = _to_record_y;
    }
